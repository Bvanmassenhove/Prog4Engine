...
namespace ImGui {
// [0..1] -> [0..1]
static double rescale(float t, float min, float max, PlotConfig::Scale::Type type) {
    switch (type) {
...
// [0..1] -> [0..1]
static double
rescale_inv(float t, float min, float max, PlotConfig::Scale::Type type) {
    switch (type) {
    case PlotConfig::Scale::Linear:
        return t;
    case PlotConfig::Scale::Log10:
    ...
    if (conf.values.count > 0) {
            ...
            case PlotConfig::Scale::Log10: {
                ...
                while (start < x_max) {
                    for (int i = 1; i < 10; ++i) {
                        ...
                        if (x < x_min) continue;
                        if (x > x_max) break;
                        float t = (float)log10(x / x_min) / (float)log10(x_max / x_min);
        ...
        for (int i = 0; i < ys_count; ++i) {
            ...
            for (int n = 0; n < res_w; n++)
            {
                ...
                const ImVec2 tp1 = ImVec2(
                    (float)rescale(t1, x_min, x_max, conf.scale.type),
        ...
        if (conf.v_lines.show) {
            for (size_t i = 0; i < conf.v_lines.count; ++i) {
                ...
                const float t1 = (float)rescale(idx * t_step, x_min, x_max, conf.scale.type);
